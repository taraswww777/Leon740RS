{
  ...
  interface IntObjById {
    [key: string]: IntPost
  }
  
  type TypeArrAllIds = string[];
  
  interface IntObjResult {
    byId: IntObjById,
    allIds: TypeArrAllIds
  }
  
  const fnGetNormalizedPosts = (unnormalizedPosts:  IntPost[]): IntObjResult => {
    const objById: IntObjById = {};
    const arrAllIds: TypeArrAllIds = [];
    const objResult: IntObjResult = {
      byId: {},
      allIds: [],
    };
  
    unnormalizedPosts.map((post) => {
      objById[post.id] = post
      arrAllIds.push(post.id);
    });
  
    objResult.byId = objById;
    objResult.allIds = arrAllIds;
  
    return objResult;
  };
  
  const objNormalizedPosts: IntObjResult = fnGetNormalizedPosts(ARR_POSTS);
  
  console.log(objNormalizedPosts);
}